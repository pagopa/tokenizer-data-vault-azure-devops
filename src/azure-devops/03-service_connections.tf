...
resource "azuredevops_serviceendpoint_aws" "uat_serviceendpoint" {
  ...
  data.aws_secretsmanager_secret_version.devops_uat.secret_string)["aws_deploy_access_key_id"]
  secret_access_key = jsondecode(
  data.aws_secretsmanager_secret_version.devops_uat.secret_string)["aws_deploy_secret_access_key"]
}
...
resource "azuredevops_serviceendpoint_aws" "prod_serviceendpoint" {
  ...
  access_key_id = jsondecode(
  data.aws_secretsmanager_secret_version.devops_prod.secret_string)["aws_deploy_access_key_id"]
  secret_access_key = jsondecode(
  data.aws_secretsmanager_secret_version.devops_prod.secret_string)["aws_deploy_secret_access_key"]
}